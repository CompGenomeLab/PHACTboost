                ...
                if k.split("|")[1].split("|")[0] in protein_to_transcript_dict.keys():
                    if transcript in protein_to_transcript_dict[k.split("|")[1].split("|")[0]]:
                        if transcript not in canonical_transcript_dict.keys():
                            ...
                            canonical_transcript_dict[transcript].append({'strand':all_proteins_dict[transcript]['strand'],'chrm':all_proteins_dict[transcript]['chrm'],'cds':all_proteins_dict[transcript]['cds'], 'protein_id':k,'protein_seq':v})
...
with open('/cta/users/abircan/id_not_found.json', 'w') as fp1:
    json.dump(id_not_found_dict, fp1)


with open('/cta/users/abircan/sequence_not_matched.json', 'w') as fp2:
